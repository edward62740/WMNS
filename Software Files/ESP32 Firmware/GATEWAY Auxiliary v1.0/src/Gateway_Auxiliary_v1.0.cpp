...
uint8_t bc_addr[] = {0x30, 0xae, 0xa4, 0x05, 0x4c, 0x0c};
uint8_t espnow_in = 0;
bool espnowrefresh = false;
bool no_fail = false;
int connection = 1;
/* Functions */
...
String hostname = "WMNS GATEWAY AUX";
...
TimerHandle_t espnowRefreshTimer;
...
#define MAXIMUM_NO_REPLY_FROM_MAIN 30000
...
void vTimerCallback5(TimerHandle_t espnowRefreshTimer)
{
  espnowrefresh = true;
}
...
void OnDataRecv(const uint8_t *mac, const uint8_t *incomingData, int len)
{
  ...
  memcpy(&espnow_in, incomingData, sizeof(espnow_in));
}
/*! setup() :: TASK
...
void setup()
{
  ...
  espnowRefreshTimer = xTimerCreate("Timer5", MAXIMUM_NO_REPLY_FROM_MAIN, pdTRUE, (void *)0, vTimerCallback5);
  xTimerStart(espnowRefreshTimer, 0);
  /* String query = "from(bucket: \"MESH\") |> range(start: -30s) |> filter(fn: (r) => r._measurement == \"GATEWAY\") |> filter(fn: (r) => r._field == \"RF Channel\") |> max()";
  ...
  while (blocker == 50)
  {
    ...
    while (result.next())
    {
      ...
      blocker = result.getValueByName("_value").getDouble();
      Serial.print("DATA");
    }
...
*/
  ...
  xTaskCreate(     // Use xTaskCreate()
      tempdisp,    // Function to be called
      "Temp disp", // Name of task
      16384,       // Stack size
  ...
  while (!no_fail)
  {
    ...
    if (espnowrefresh == true)
    {
      if (espnow_in != 201)
      {
        Serial.print("MAIN GATEWAY FAILED");
        no_fail = true;
      }
      else
      {
  ...
  vTaskDelete(tmp);
  xTimerDelete(espnowRefreshTimer, 0);
  ...
  while (true)
  {
    ...
    if (uxQueueMessagesWaiting(stat_queue) > 0)
    {
      ...
      if (xQueueReceive(stat_queue, (void *)&STAT_OUT, 1) == pdPASS)
      {
        ...
        float mul;
        if (((STAT_OUT.uid.toInt()) >= 0 && (STAT_OUT.uid.toInt()) <= 255))
        {
          mul = alive_rate_r;
        }
        else
    ...
    if (update_self == true)
    {
      ...
      GATEWAY.addField("G Device Count", (self.COUNT_NUM_G));
...
void tempdisp(void *pvParameters)
{
  ...
  while (1)
  {
    ...
    for (int i = 0; i < 27; i++)
    {
      display.fillCircle(80 - i, 37, 2, SSD1306_WHITE);
      display.display();
      delay(5);
      display.fillCircle(80 - i, 37, 2, SSD1306_BLACK);
    }
    for (int i = 27; i > 0; i--)
    {
      display.fillCircle(80 - i, 37, 2, SSD1306_WHITE);
    ...
    display.fillRect(15, 40, 105, 30, BLACK);
    ...
    display.setTextSize(1);
    display.drawBitmap(35, 30, aux_linked, 16, 14, SSD1306_WHITE);
    display.setCursor(33, 30);
    display.print("0");
    display.drawBitmap(85, 30, aux_linked, 16, 14, SSD1306_WHITE);
