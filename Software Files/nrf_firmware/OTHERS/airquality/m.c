...
static uint8_t gas_calib_count = 0;
static uint32_t gas_calib_value = 0;
...
void HardFault_Handler(void)
{
    ...
    mesh_tx_battery((unsigned int)ia);
    mesh_wait_tx();
...
void GPIOTE_IRQHandler(void)
{
    ...
    if(NRF_GPIOTE->EVENTS_PORT)
    {
        ...
        int32_t temp    = BME680_get_temperature();
        uint32_t hum    = BME680_get_humidity();
        uint32_t press  = BME680_get_pressure();
        uint32_t iaq = 0;
        uint16_t raw_light = opt3001_read();
        uint32_t light = opt3001_conv(raw_light);
        if (gas_calib_count < 5) {
            gas_calib_value = (gas_calib_value + BME680_get_gas()) / 2;
            gas_calib_count++;
        }
        else {
            iaq = BME680_get_gas();
        ...
        NRF_LOG_DEBUG("gas_calib_count");
        NRF_LOG_DEBUG("gas value %d", gas_calib_value);
        mesh_tx_iaq(temp, hum, press, light, iaq);
...
nrf_gpio_cfg_output(4);
    ...
    if( ((cycle_count+offset_env) % period_env)==0)
    {
        opt3001_init(&m_twi);
        BME680_init(&m_twi);

        if (gas_calib_count < 5) {
            BME680_measure(true, 0);
        }
        else {
            BME680_measure(false, gas_calib_value);
        }
