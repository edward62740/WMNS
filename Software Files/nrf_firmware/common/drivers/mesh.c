...
void mesh_tx_lts(int32_t temp, uint32_t hum)
{
    uint8_t data[14];
    data[0] = 0xFF & (uint8_t)(temp >> 24);
    data[1] = 0xFF & (uint8_t)(temp >> 16);
    ...
    data[6] = 0xFF & (uint8_t)(hum >> 8);
    data[7] = 0xFF & (uint8_t)(hum);
    mesh_bcast_data(MESH_PID_LTS, data, 8);
}
...
int rx_lts(char *p_msg, uint8_t *data, uint8_t size) {
	char *p_start = p_msg;
	if (size != 8) {
		...
		hum |= data[6] << 8;
		hum |= data[7];
		uint32_t msh = hum / 1000;
		uint32_t lsh = hum % 1000;
		p_msg += sprintf(p_msg, "hum:%lu.%03lu", msh, lsh);
		return (p_msg - p_start);
	}
...
void mesh_parse(message_t *msg, char *p_msg)
{
    ...
    case MESH_PID_LTS:
    {
        p_msg += rx_lts(p_msg, msg->payload, msg->payload_length);
    }
    break;
